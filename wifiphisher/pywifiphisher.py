...
def parse_args():
    ...
    parser.add_argument(
        "-rP",
        "--report-path",
        help="Determine the full path of the generated report file",
        default=None)
    parser.add_argument(
        "-cP",
        "--credential-log-path",
...
class WifiSpyEngine:
    """
    This is the main engine for WifiSpy.
    """
    ...
    def __init__(self):
        ...
        self.victim_manager = victim.VictimManager.get_instance()
        self.report = None  # Will be initialized in start()

    def stop(self):
        # Try to remove if it is created
        ...
        if os.path.isfile('/tmp/wifiphisher-hostapd.tmp'):
            os.remove('/tmp/wifiphisher-hostapd.tmp')

        if self.opmode.internet_sharing_enabled():
            self.fw.restore_redirect_rules(self.opmode.get_internet_interface())
            if self.opmode.get_layers_present()[0] == 3:
                self.fw.restore_layer3_rules()

        # Close downstream
        if self.access_point.dhcp_conf and self.access_point.dhcp_conf.startswith("/tmp/"):
            os.remove(self.access_point.dhcp_conf)
            self.access_point.dhcp_conf = None
            
        # Save the report before shutting down
        if hasattr(self, 'report'):
            report_path = self.report.save_report()
            if report_path:
                print("[" + G + "+" + W + "] Report saved to: " + report_path)
                
            # Export credentials to CSV if any were captured
            if len(self.report.report_data["credentials"]) > 0:
                csv_path = self.report.export_csv()
                if csv_path:
                    print("[" + G + "+" + W + "] Credentials exported to: " + csv_path)
                
        try:
            self.network_manager.off()
    ...
    def start(self):
        ...
        print('[' + T + '*' + W + '] Starting WifiSpy %s (%s) at %s' %
              (VERSION, WEBSITE, today))

        # Show some welcome message
        print('[' + T + '*' + W + \
            '] WifiSpy - Professional Wi-Fi Security Assessment Tool')
        ...
        if args.report_path:
            self.report = reporting.Report(args.report_path)
        else:
        ...
        if target_ap_mac:
            self.report.add_target(essid, target_ap_mac, channel, enctype)
        ...
        # Connect the report to the victim manager
        self.victim_manager.set_report(self.report)


def run():
    try:
        # Mirror all arguments that are not caught by ArgumentParser to the engine
        # as they might be wifiphisher arguments for extension operations
        _, args = argparse.ArgumentParser().parse_known_args()
        if args != []:
            new_args = []
            for unparsed_arg in args:
                if "=" in unparsed_arg:
                    new_args.append(unparsed_arg)
                else:
                    router_elt = unparsed_arg + "=true"
                    new_args.append(router_elt)
            args = []

            for unparsed_arg in new_args:
                try:
                    arg_key, arg_value = unparsed_arg.split("=", 1)
                    args.append(arg_key)
                    args.append(arg_value)

                except ValueError:
                    args.append(unparsed_arg)
        ...
        # setup the logging configuration
        setup_logging(args)
        logger.info("Starting WifiSpy %s at %s", VERSION, time.strftime("%Y-%m-%d %H:%M"))
        # We need to initialize the game engine
        wifispy_engine = WifiSpyEngine()
        wifispy_engine.start()

    except KeyboardInterrupt:
        print(R + "\n[" + R + "!" + R + "] " + O + "Closing")
        logger.info('Closing WifiSpy. Captured credentials: %s', str(wifispy_engine.victim_manager.victims))
        # Make sure to save the report
        if hasattr(wifispy_engine, 'report'):
            wifispy_engine.report.save_report()
        sys.exit(0)
    except EOFError:
        print(R + "\n[" + R + "!" + R + "] " + O + "Closing")
        logger.info('WifiSpy stopped. Captured credentials: %s', str(wifispy_engine.victim_manager.victims))
        # Make sure to save the report
        if hasattr(wifispy_engine, 'report'):
            wifispy_engine.report.save_report()
        sys.exit(0)
    except Thread.error as err:
        print(R + "\n[" + R + "-" + R + "] " + O + "Closing because of an error.")
        logger.info('Closing WifiSpy because of an error. Captured credentials: %s', 
                    str(wifispy_engine.victim_manager.victims))
        logger.error(err)
        # Make sure to save the report
        if hasattr(wifispy_engine, 'report'):
            wifispy_engine.report.save_report()
